...
from src import ui, entity, data, Space, events
...
class Game:
    def __init__(self, config: AppConfig, clock: pg.time.Clock) -> None:
        ...
        self.scroll: list[float] = [0.0, 0.0]
        self.zoom = 0.2

        center_x = self.WIDTH//2
        center_y = self.HEIGHT//2

        self.space = Space((self.config.Game.AccelGravity,
                           self.config.Game.AngleGravity), 2)

        self.surface = pg.Surface((self.WIDTH, self.HEIGHT))

        maps: list[data.MapJSON] = data.get_maps(
            os.path.join(config.Assets.Folder, config.Assets.Maps))
        polygons: list[data.PolygonJSON] = data.get_polygons(
            os.path.join(config.Assets.Folder, config.Assets.Polygons))

        """OBJECTS"""
        lander_image = pg.image.load(os.path.join(
        ...
        if isinstance(maps, list):
            mount = entity.TerrainSprite(
                "white", maps[0].Heights, maps[0].Vertical_Stretch, maps[0].Horizontal_Stretch, self.config.Game.TerrainThickness)
            static = entity.StaticObject(0, 0, 0, 0)
            terrain = entity.Entity(static, mount)
            self.space.add_object(terrain)

        """LISTENERS"""
        self.event_checker = events.EventsChecker()
        self.event_checker.add_event(
            events.Event(pg.K_a, self.lander.turn_left))
        self.event_checker.add_event(
            events.Event(pg.K_d, self.lander.turn_right))
        self.event_checker.add_event(events.Event(pg.K_w, self.lander.thrust))
        ...
        def is_in_bounds_x() -> bool:
            ...
            # type: ignore
            return screen_x not in screen_range_x and np.sign(screen_x) == np.sign(phys.vx)
        ...
        self.event_checker.add_event(events.Event(is_in_bounds_x, add_x_vel))
        self.event_checker.add_event(events.Event(is_in_bounds_y, add_y_vel))
        ...
        Padding = 30
        WindowWithPadding = pg.Rect(
            Padding, Padding, self.WIDTH - Padding, self.HEIGHT - Padding)

        # Top Left Heads Up
        score = ui.Text("SCORE  0000", SystemFontSmall,
                        (0, 0), (500, 50), "center_left")
        time = ui.Text("TIME   0:00", SystemFontSmall,
                       (0, 0), (500, 50), "center_left")
        fuel = ui.Text("FUEL   0000", SystemFontSmall,
                       (0, 0), (500, 50), "center_left")
        top_left_list = ui.MultiLineText(
            [score, time, fuel], pg.Rect(0, 0, 300, 100))
        ...
        # Top Right Heads Up
        altitude = ui.Text("ALTITUDE          0000",
                           SystemFontSmall, (0, 0), (500, 50), "center_right")
        horizontal_speed = ui.Text(
            "HORIZONTAL SPEED  0000", SystemFontSmall, (0, 0), (500, 50), "center_right")
        vertical_speed = ui.Text(
            "VERTICAL SPEED    0000", SystemFontSmall, (0, 0), (500, 50), "center_right")
        top_right_list = ui.MultiLineText(
            [altitude, horizontal_speed, vertical_speed], pg.Rect(0, 0, 400, 100))
        ...
        # Low On Fuel Text
        text = ui.Text("LOW ON FUEL", SystemFontSmall,
                       (0, 0), None, "center_center")
        wrap = ui.Wrapper(text, pg.Rect(
            0, 0, self.WIDTH, self.HEIGHT), "center_center")
