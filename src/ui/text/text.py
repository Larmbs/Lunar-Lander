...
class Text(Display):
    ...
    def calc_offset(self) -> None:
        ...
        if hor == "center":
            x_off = (self.rect.width - self.text_size[0])//2
        elif hor == "right":
            x_off = self.rect.width - self.text_size[0]
        # check for vertical alignment specification
        if vert == "center":
            y_off = (self.rect.height - self.text_size[1])//2
        elif vert == "bottom":
            y_off = self.rect.height - self.text_size[1]
