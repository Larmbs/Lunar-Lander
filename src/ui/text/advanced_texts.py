...
class Wrapper(Display):
    ...
    def calc_offset(self):
        ...
        if hor == "center":
            x_off = (self.rect.width - item_size.width)//2
        elif hor == "right":
            x_off = self.rect.width - item_size.width
        # check for vertical alignment specification
        if vert == "center":
            y_off = (self.rect.height - item_size.height)//2
        elif vert == "bottom":
            y_off = self.rect.height - item_size.height
...
class MultiLineText(Display):
    def __init__(self, displays: list[Display], rect: pg.Rect | None = None, line_height: int | None = None) -> None:
        ...
        self.surface = pg.Surface(
            (self.rect.width, self.rect.height), pg.SRCALPHA, 32)
        self.surface.convert_alpha()
    ...
    def update(self) -> None:
        for index, display in enumerate(self.displays):
            display.rect = pg.Rect(self.rect.left, self.rect.top +
                                   index * self.line_height, self.rect.width, self.line_height)
            display.update()
